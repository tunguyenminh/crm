...
<div class="modal-body">
    ...
    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('module_user.password')</label>
        <div class="col-sm-9">
    ...
    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('app.address')</label>
        <div class="col-sm-9">
            <input type="text" name="address" class="form-control" value="{{ isset($userDetails->address) ? $userDetails->address : '' }}">
        </div>
    </div>

    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('module_user.country')</label>
        <div class="col-sm-9">
            <input type="text" name="country" class="form-control" value="{{ isset($userDetails->country) ? $userDetails->country : '' }}">
        </div>
    </div>

    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('module_user.zipCode')</label>
        <div class="col-sm-9">
            <input type="text" name="zip_code" class="form-control" value="{{ isset($userDetails->zip_code) ? $userDetails->zip_code : '' }}">
        </div>
    </div>

    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('app.status')</label>
        <div class="col-md-9">
            {{ Form::select('status', $statusArray, $userDetails->status, ['class' => 'form-control']) }}
        </div>
    </div>

    @if($user->ability('admin', 'assign_role') && $userDetails->id != 1 && $userDetails->id != $user->id)
    <div class="form-group row">
        <label class="col-sm-3 col-form-label">@lang('module_user.userRole')</label>
        <div class="col-md-9">
            <select class="form-control" name="role_id">
                <option value="">Select Role...</option>
                @foreach($roles as $role)
                    <option value="{{ $role->id }}" @if(isset($userDetails->role()->id) && $userDetails->role()->id == $role->id) selected @endif>{{ $role->display_name }}</option>
                @endforeach
            </select>
        </div>
    </div>
