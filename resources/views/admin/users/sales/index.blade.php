...
<script>
    ...
    $(function() {
        table.dataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.get-sales-users') !!}',
            aaSorting: [[0, "desc"]],
            language: {
        ...
        function deleteModal(id) {
            swal({
                ...
                if (isConfirm) {
                    ...
                    var url = "{{ route('admin.sales-users.destroy',':id') }}";
    ...
    @if($user->ability('admin', 'sales_member_create,sales_member_edit'))
        ...
        @if($user->ability('admin', 'sales_member_create'))
            function addModal () {
            $.ajaxModal('#addEditModal','{{ route('admin.sales-users.create') }}');
        }
        @endif

        @if($user->ability('admin', 'sales_member_edit'))
            function editModal (id) {
            var url = '{{ route('admin.sales-users.edit', ':id') }}';
            url      = url.replace(':id',id);
        ...
        if(typeof id != 'undefined'){
            var url  ="{{route('admin.sales-users.update',':id')}}";
            url      = url.replace(':id',id);
        }
        @endif
        ...
        @if($user->ability('admin', 'sales_member_create'))
        if (typeof id == 'undefined'){
            url = "{{ route('admin.sales-users.store') }}";
        }
        @endif
